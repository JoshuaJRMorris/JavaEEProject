...
public class TaskShowServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet TaskShowServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!(request.getParameter("submit") == null)){
            for (int i = 0; i < tasks.size(); ++i){
                if (tasks.get(i).getName().equals(request.getParameter("submit"))){
                    session.setAttribute("currentTask", tasks.get(i));
                }
            }
        }
